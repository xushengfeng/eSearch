 ...
 */
function stack_add() {
    if (undo_stack[undo_stack_i] == editor_get())
        return;
...
var line = {};
var pg_to_line = {};
function push_p_l(i, v) {
    if (!pg_to_line[i])
        pg_to_line[i] = [];
    if (!pg_to_line[i].includes(v))
        pg_to_line[i].push(v);
}
var line_height = 24;
function add_line() {
    ...
    for (let pg of pg_l) {
        ...
        if (line_height * 0.5 <= pg.offsetHeight && pg.offsetHeight <= line_height * 1.5) {
                    ...
                    for (let j = k_min; j <= k_min + d_k; j++) {
                        ...
                        if (w_l[j + 1]) {
                            ...
                            if (n_w.offsetTop != h) {
                                ...
                                // 上一行
                                if (!line[w_top])
                                    line[w_top] = { min: { i: 0, l: 0 }, max: { i: NaN, r: NaN }, pg: i };
...
document.getElementById("top").addEventListener("mousedown", (e) => {
    ...
    if ((t_s.end.pg - t_s.start.pg >= 2 && t_s.start.pg <= cursor.pg && cursor.pg <= t_s.end.pg) /* >=3 */ ||
...
var replace_input = document.getElementById("replace_input");
var find_t = document.querySelector(".find_t > span");
...
function find() {
    ...
    try {
        ...
        for (i in text_l) {
            if (text_l[i])
                t_l.push(word_to_span_string_split(text_l[i]));
            if (match_l[i])
                t_l.push(`<span class="find_h">${word_to_span_string_split(match_l[i])}</span>`);
        }
...
function find_l_n(a) {
    ...
    document.getElementById("text_out").scrollTop = l[find_l_n_i].offsetTop - 48 - 16;
}
...
function render_history() {
    ...
    if (Object.keys(history_list).length == 0)
        document.querySelector("#history_list").innerHTML = "暂无历史记录";
    for (let i in history_list) {
        var t = html_to_text(history_list[i].text).split(/[\r\n]/g);
        var div = document.createElement("div");
        div.id = i;
        var f = require("./lib/time_format");
        div.innerHTML = `<div class="history_title"><span>${f(store.get("时间格式"), new Date(Number(i) - 0))}</span><button><img src="./assets/icons/close.svg" class="icon"></button></div><div class="history_text">${t.splice(0, 3).join("<br>") + (t.length > 3 ? "..." : "")}</div>`;
        ...
        fs.writeFile(tmp_text_path, data, () => {
            ...
            file_watcher = fs.watch(tmp_text_path, () => {
                fs.readFile(tmp_text_path, "utf8", (e, data) => {
                    ...
                    if (cu.pg > get_pg_max())
                        cu.pg = get_pg_max();
                    if (cu.of > get_w_max(cu.pg))
                        cu.of = get_w_max(cu.pg);
                    cursor = cu;
                    editor_i(cursor.pg, cursor.of);
                });
            });
...
function new_tab(id, url) {
    let li = document.getElementById("tab").cloneNode(true);
    li_list.push(li);
...
function baidu(image, callback) {
    ...
    post("https://graph.baidu.com/upload", { headers: { "content-type": "application/x-www-form-urlencoded" }, body: data }, (err, result) => {
        if (err)
...
async function local_ocr(arg, callback) {
    ...
    var detp = store.get("OCR.det") || path.join(model_path, "inference/ch_PP-OCRv2_det_infer.onnx"), recp = store.get("OCR.rec") || path.join(model_path, "inference/ch_PP-OCRv2_rec_infer.onnx"), 字典 = store.get("OCR.字典") || path.join(model_path, "ppocr_keys_v1.txt");
...
function online_ocr(type, arg, callback) {
    var client_id = store.get(`在线OCR.${type}.id`), client_secret = store.get(`在线OCR.${type}.secret`);
    ...
    function baidu_ocr() {
        fetch(`https://aip.baidubce.com/oauth/2.0/token?grant_type=client_credentials&client_id=${client_id}&client_secret=${client_secret}`, { method: "GET" })
            .then((t) => t.json())
            .then((result) => {
