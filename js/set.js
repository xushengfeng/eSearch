...
document.querySelector("#选区颜色 > span").style.backgroundImage = `linear-gradient(${store.get("选区颜色")}, ${store.get("选区颜色")}), url('assets/tbg.svg')`;
...
document.querySelector("#遮罩颜色 > input").oninput = () => {
    document.querySelector("#遮罩颜色 > span").style.backgroundImage = `linear-gradient(${document.querySelector("#遮罩颜色 > input").value}, ${document.querySelector("#遮罩颜色 > input").value}), url('assets/tbg.svg')`;
};
document.querySelector("#选区颜色 > input").oninput = () => {
    document.querySelector("#选区颜色 > span").style.backgroundImage = `linear-gradient(${document.querySelector("#选区颜色 > input").value}, ${document.querySelector("#选区颜色 > input").value}), url('assets/tbg.svg')`;
};
...
function show_f_time() {
    ...
    document.getElementById("保存文件名称_p").innerText = `${document.getElementById("保存文件名称前缀").value}${f(document.getElementById("保存文件名称时间").value, save_time)}${document.getElementById("保存文件名称后缀").value}`;
}
show_f_time();
document.getElementById("保存文件名称前缀").style.width = `${document.getElementById("保存文件名称前缀").value.length || 1}em`;
document.getElementById("保存文件名称后缀").style.width = `${document.getElementById("保存文件名称后缀").value.length || 1}em`;
...
function save_setting() {
    ...
    store.set("插件.加载后", document.getElementById("plugin").value.trim().split("\n"));
    ...
    if (user_data_path_inputed)
        fs.writeFile("preload_config", document.getElementById("user_data_path").value, (e) => {
            if (e)
                throw new Error(t("保存失败，请确保软件拥有运行目录的修改权限，或重新使用管理员模式打开软件"));
        });
}
// 查找
...
document.getElementById("user_data_path").value =
...
ipcRenderer.on("setting", (err, t, id, r) => {
    if (t == "open_dialog") {
        switch (id) {
            case "ocr_det":
                if (!r.canceled) {
                    document.getElementById("ocr_det").value = r.filePaths[0];
                }
                break;
            case "ocr_rec":
                if (!r.canceled) {
                    document.getElementById("ocr_rec").value = r.filePaths[0];
                }
                break;
            case "ocr_字典":
                if (!r.canceled) {
