...
ipcRenderer.on("reflash", (a, data, ww, hh, act) => {
    console.log(data);
    function to_canvas(canvas, img, w, h) {
        canvas.width = w;
        canvas.height = h;
...
function edge() {
    ...
    let dst = new cv.Mat();
    let c_min = store.get("框选.自动框选.最小阈值"), c_max = store.get("框选.自动框选.最大阈值");
...
hotkeys.filter = (event) => {
    ...
    var v = !(event.target.isContentEditable || tagName == "INPUT" || tagName == "SELECT" || tagName == "TEXTAREA") ||
...
识图引擎.oninput = () => {
    if (store.get("以图搜图.记住"))
        store.set("以图搜图.记住", 识图引擎.value);
    tool_search_f();
};
...
function track_location() {
    ...
    let x = document.getElementById("tool_bar").offsetLeft < final_rect[0] &&
...
var undo_stack = [{ rect: 0, canvas: 0 }], rect_stack = [[0, 0, main_canvas.width, main_canvas.height]], canvas_stack = [{}];
var undo_stack_i = 0;
var ratio = window.devicePixelRatio;
...
function in_edge(e) {
    ...
    for (const i of edge_rect) {
        let x0 = i.x, y0 = i.y, x1 = i.x + i.width, y1 = i.y + i.height;
...
function wh_bar(final_rect) {
    ...
    if (final_rect[1] - (dh * ratio + 10) >= 0) {
        ...
        if (final_rect[1] + final_rect[3] + 10 + document.getElementById("clip_wh").offsetHeight <=
...
function final_rect_fix() {
    ...
    var w = Math.max(x0, x1) - x, h = Math.max(y0, y1) - y;
    // 移出去移回来保持原来大小
    if (x < 0)
        w = x = 0;
    if (y < 0)
        h = y = 0;
    if (x > main_canvas.width)
        x = x % main_canvas.width;
    if (y > main_canvas.height)
        y = y % main_canvas.height;
    if (x + w > main_canvas.width)
        w = main_canvas.width - x;
    if (y + h > main_canvas.height)
        h = main_canvas.height - y;
    final_rect = [x, y, w, h];
}
// 判断光标位置并更改样式
// 定义光标位置的移动方向
function is_in_clip_rect(event) {
    ...
    var x = now_canvas_position[0], y = now_canvas_position[1];
    var x0 = final_rect[0], x1 = final_rect[0] + final_rect[2], y0 = final_rect[1], y1 = final_rect[1] + final_rect[3];
    // 如果全屏,那允许框选
...
function move_rect(o_final_rect, oe, e) {
    ...
    if (final_rect[0] + final_rect[2] > main_canvas.width)
        final_rect[2] = main_canvas.width - final_rect[0];
    if (final_rect[1] + final_rect[3] > main_canvas.height)
        final_rect[3] = main_canvas.height - final_rect[1];
    final_rect_fix();
    draw_clip_rect();
}
/**
...
function his_push() {
    ...
    if (undo_stack_i != undo_stack.length - 1 && undo_stack.length >= 2)
    ...
    if (rect_stack[rect_stack.length - 1] + "" != final_rect_v + "")
        rect_stack.push(final_rect_v);
    if (JSON.stringify(canvas_stack[canvas_stack.length - 1]) != JSON.stringify(canvas))
...
function undo(v) {
    ...
    follow_bar();
    if (fabric_canvas)
        fabric_canvas.loadFromJSON(canvas_stack[c.canvas]);
}
...
var free_i_els = document.querySelectorAll("#draw_free_i > lock-b");
// 笔
var pencil_el = document.getElementById("draw_free_pencil");
pencil_el.oninput = () => {
...
// 橡皮
var eraser_el = document.getElementById("draw_free_eraser");
eraser_el.oninput = () => {
...
// 刷
var free_spray_el = document.getElementById("draw_free_spray");
...
function free_shadow() {
    shadow_blur = Number(document.querySelector("#shadow_blur > range-b").value);
...
document.getElementById("draw_shapes_i").onclick = (e) => {
    ...
    if (el.id != "draw_shapes_i") {
        ...
        if (store.get(`图像编辑.形状属性.${shape}`)) {
            ...
            document.querySelector("#draw_stroke_width > range-b").value = stroke_width;
        }
    }
    exit_free();
    exit_filter();
...
// 输入颜色
var color_alpha_input_1 = document.querySelector("#draw_color_alpha > range-b:nth-child(1)");
...
var color_alpha_input_2 = document.querySelector("#draw_color_alpha > range-b:nth-child(2)");
...
function color_bar() {
    ...
    function c_color(el) {
        ...
        if (color_m == "fill")
            color_alpha_input_1.value = "100";
        if (color_m == "stroke")
            color_alpha_input_2.value = "100";
    }
}
color_bar();
document.querySelector("#draw_stroke_width > range-b").oninput = () => {
    set_f_object_v(null, null, Number(document.querySelector("#draw_stroke_width > range-b").value));
};
/** 鼠标点击后，改变栏文字样式 */
...
function get_filters() {
    ...
    console.log(f);
    document.querySelector("#draw_filters_pixelate > range-b").value = String(f[0]?.blocksize || 0);
    document.querySelector("#draw_filters_blur > range-b").value = String(f[1]?.blur * 100 || 0);
    document.querySelector("#draw_filters_brightness > range-b").value = String(f[2]?.brightness || 0);
    document.querySelector("#draw_filters_contrast > range-b").value = String(f[3]?.contrast || 0);
    document.querySelector("#draw_filters_saturation > range-b").value = String(f[4]?.saturation || 0);
    document.querySelector("#draw_filters_hue > range-b").value = String(f[5]?.rotation || 0);
    document.querySelector("#draw_filters_gamma > range-b:nth-child(1)").value = String(f[6]?.gamma[0] || 1);
    document.querySelector("#draw_filters_gamma > range-b:nth-child(2)").value = String(f[6]?.gamma[1] || 1);
    document.querySelector("#draw_filters_gamma > range-b:nth-child(3)").value = String(f[6]?.gamma[2] || 1);
    document.querySelector("#draw_filters_noise > range-b").value = String(f[7]?.noise || 0);
    var gray = f[8]?.mode;
    switch (gray) {
        case "average":
...
document.querySelector("#draw_filters_pixelate > range-b").oninput = () => {
    var value = Number(document.querySelector("#draw_filters_pixelate > range-b").value);
    if (value != 0) {
...
document.querySelector("#draw_filters_blur > range-b").oninput = () => {
    var value = Number(document.querySelector("#draw_filters_blur > range-b").value) / 100;
    if (value != 0) {
...
// fabric命令行
var draw_edit_input_el = document.querySelector("#draw_edit input");
