...
**eSearch**目前支持的在线 OCR 服务是**百度 OCR**、**有道 OCR**和大模型OCR。部分使用OCR的功能可能只能使用离线OCR。
...
默认使用 CPU 运行模型，还支持 WebGPU CUDA（linux）、coreML（mac）、DirectML（Windows）。**理论上**如果你的设置支持以上三种后端，应该可以更快运行模型。部分模型或设备不支持一些后端，如有错误，请改回 CPU 后端。
