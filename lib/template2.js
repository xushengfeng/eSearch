...
class range_b extends HTMLElement {
    ...
    connectedCallback() {
        ...
        input.style.backgroundSize = ((input.value - input.min) / (input.max - input.min)) * 100 + "%";
        ...
        p[0].oninput = () => {
            ...
            if (p[0].innerText - 0 < input.min || p[0].innerText - 0 > input.max) return;
        ...
        p[0].onblur = () => {
            if (p[0].innerText === "") p[0].innerText = this.getAttribute("value");
        };
    }
    ...
    c_value(value) {
        ...
        input.min = this.getAttribute("min") || 0;
        input.max = this.getAttribute("max") || 100;
        input.style.backgroundSize = ((value - input.min) / (input.max - input.min)) * 100 + "%";
    }
    c_t(value) {
        var t = this.getAttribute("text") || "";
        var p = this.querySelectorAll("div > p");
